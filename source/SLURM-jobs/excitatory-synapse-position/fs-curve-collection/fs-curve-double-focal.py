    ...
    for j in range(num_branch):
        ...
        if not weighted:
            ...
            for i in range(branches[j].nseg):
                n_sp = np.floor(9*(branches[j].nseg - i)/branches[j].nseg) + 5
                for n in range(n_sp):
                    # Create head and neck biophysical parameters
                    spine_head_temp.append(h.Section())
                    spine_neck_temp.append(h.Section())
...
def increase_syn(branch, 
    ...
    loc_1 = int(np.floor(len(syn_ampa[0])*loc[0]))
    loc_2 = int(np.floor(len(syn_ampa[0])*loc[1]))
        ...
        for i in range(n_syn+1):
            ...
            if i != 0:
                ...
                if (i % 2) == 1:
                    ...
                    ncstim.append(h.NetCon(syn_stims, syn_ampa[0][loc_1], 0, 0, g[0]))
                    ncstim.append(h.NetCon(syn_stims, syn_nmda[0][loc_1], 0, 0, g[1])) # gMax
                else:
                    # Apply stimulus to excitatory dendrite synapse (increasing the # of synapses)
                    ncstim.append(h.NetCon(syn_stims, syn_ampa[0][loc_2], 0, 0, g[0]))
                    ncstim.append(h.NetCon(syn_stims, syn_nmda[0][loc_2], 0, 0, g[1])) # gMax
...
filename = "fs-collection-position/double-focal-raw/trial-" + str(trial) + ".csv"
